...
export async function downloadOrderReceipt(
  ...
  // ============================ HEADER ============================
  let y = 16;
  const brandX = logo ? margin + 24 : margin;
  if (logo) {
    doc.addImage(logo, 'PNG', margin, 13, 20, 17);
  }
  text('bold', 19, DARK);
  ...
  y += 7;
  totalRow('TOTAL', money(order.total), true, ORANGE, 13);
  y += 14;
  ...
  // ============================ FOOTER ============================
  text('bold', 10, DARK);
  doc.text('Thank you for ordering from grillsJunction!', pageW / 2, y, { align: 'center' });
  y += 5.5;
  text('normal', 8.5, MUTED);
